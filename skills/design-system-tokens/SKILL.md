...
description: The Design System is a dual-file contract (DESIGN.md + theme.css) of fixed design tokens exposed as CSS variables. Use this skill to create/update the Design token values or document their usage to create components.
---

# 1. Overview

This skill document the usage of design tokens in our design system applied approach. 

## The dual-file contract

The Design System is composed of two main files : 

- **`DESIGN.md`**: The agent-readable documentation of your design system. This markdown file is divided in two parts : the _design tokens_ living inside the YAML front-matter, and the prose sections documenting their usage to create components. The full specifications for this file can be found here: [google-labs-code/design.md](https://github.com/google-labs-code/design.md).
...
- The full list of usable design token is **fixed** and documented. 
...
- Design tokens inside `DESIGN.md` front matter are structured into categories, sub-categories that make them accessible by their path.
...
**Principle:** Use a geometric scale (e.g. `1.125` Minor Second, `1.25` Major Third, `1.333` Perfect Fourth, `1.5` Perfect Fifth, `1.618` Golden Ratio) or a linear scale (e.g. `2px` steps) to generate the values from `xs` through `display` : `[xs, sm, md, lg, xl, xxl, display]`
**Formula:** 
  - for a geometric scale with ratio `r` the size of each step is `size(step) = 1rem * r^step`.
  - for a linear scale with ratio `r` the size of each step is `size(step) = 1rem + r*step`.
**Base:** The root base font size (`--font-size-base`) is always equal to `1rem` and is the step 0. It is usually applied to size `md` (`--font-size-md`). But step 0 could be applied to `sm` or `lg` for specific use cases.


| Token path | CSS variable | Required | Value | Description |
|---|---|---|---|---|
|   | `--font-size-xs` | Y | `0.444rem` | Captions, metadata, timestamps |
|   | `--font-size-sm` | Y | `0.667rem` | Secondary text, list rows |
|   | `--font-size-md` | Y | `1rem` | Body text |
...
Check each of this rules after any edition to the Design System files :

* **Undocumented tokens are FORBIDDEN** in the stylesheet or to the front matter. Contrary to the Google Labs approach, only the tokens listed in sections 3–8 are permitted in our **Design System Applied** approach.
